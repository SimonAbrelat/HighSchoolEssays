...
from typing import List # Type Inference
from math import pow # Poewr function
import numpy as np # Arrays and matrices

p: List[float] = [0, 3] # The control points
d: List[float] = [0, 1] # the derivative at said control points
...
def hermite(t1: float, t2: float, dt1: float, dt2: float, s: float) -> float:
    ...
    h = np.matrix([[ 2, -2,  1,  1], # weight matrix that represent
                   [-3,  3, -2, -1], # the basis functions
                   [ 0,  0,  1,  0],
                   [ 1,  0,  0,  0]])
    # multiplies the matrices all together and gets the scalar from them 
